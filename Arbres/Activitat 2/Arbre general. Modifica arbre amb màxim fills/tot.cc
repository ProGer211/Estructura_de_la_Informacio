 ...
 private:
   ...
   static void arbre_max_fills(node* p,node* pare);
 ...
 template <typename T>
  void Arbre<T>::arbre_max_fills(node* p,node* pare)
  {
  int i(0),fill(0);
   while(p!= nullptr)
   {
   arbre_max_fills(p->primf,p);
   if(i == 0)fill = p->info;
   else if(fill < p->info) fill = p->info; 
   i++;
   p = p->seggerm;   
   }
   if(pare->primf != nullptr)pare->info = fill;
  }
  
  template <typename T>
   void Arbre<T>::arbre_max_fills()
   {
   arbre_max_fills(_arrel->primf,_arrel);
   }
