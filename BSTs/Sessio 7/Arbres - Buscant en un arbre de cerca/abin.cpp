...
class Abin {
  private:
    struct node {
      ...
      node* f_dret;
      T info;
      //node(const T &k,node* esq = nullptr,node* dret = nullptr);
    };
    node* _arrel;
    ...
    static bool consulta_bst(node* n,const T &k);
  public:
    //constructors
    Abin(): _arrel(NULL) {};
    /* Pre: cert */
...
template <typename T>
bool Abin<T>::consulta_bst (node* n ,const T &k ) 
{
if(n!= nullptr)
{
 ...
 {
  trobat = consulta_bst(n->f_esq,k);
  trobat = consulta_bst(n->f_dret,k);
 }
}
return trobat;
...
{
bool trobat = false;
trobat = consulta_bst(_arrel,k);
}


template <typename T>
void Abin<T>::crea(int n)
{
if(n != -1)
{
crea Abin<T>(,,);
...
/*template <typename T>
Abin<T>::node::node(const T &k,node* esq,node* dret) : info(k),f_esq(esq),f_dret(dret){ // es per crear el node directament amb new node(k);
}*/
...
if(n!= nullptr)
{
if(n->info > k)  n->f_esq = insereix_bst(n->f_esq,k);
else if(n->info < k) n->f_dret = insereix_bst(n->f_dret,k);
return n;
}
else
{
...
int n;
cin >> n;
Abin<int> bst = crea<int>(n);
while(cin >> n){
cout << n << " " << bst.consulta_bst(n) << endl;
