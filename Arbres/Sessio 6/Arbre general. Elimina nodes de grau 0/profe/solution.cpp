#include <cstdlib>
 ...
 private:
   ...
   static node* elimina_grau0(node *n);
 };
 
 // Aquí va la implementació del mètode elimina_grau0
 
 template <typename T>
 typename Arbre<T>::node* Arbre<T>::elimina_grau0(node *n)
 {
  if(n!=nullptr){
 	if(n->primf == nullptr)
 	{
 	node *elim = n;
 	n = elimina_grau0(n->seggerm);
 	delete elim;
 	}
 	else {
 	n->primf = elimina_grau0(n->primf);
 	n->seggerm = elimina_grau0(n->seggerm);
 	}
